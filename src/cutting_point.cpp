 ...
 * @brief Implementacion de algoritmos de punto de corte optimo (JAN1PT/JAN11PT).
 *
...
/**
 * @brief Estructura interna para un corte candidato.
 */
struct CutCandidate {
...
/**
 * @brief Resultado de evaluacion para una polaridad.
 *
...
/**
 * @brief Genera puntos de corte candidatos.
 *
...
/**
 * @brief Cuenta clasificacion para todos los cortes con una polaridad dada.
 *
 * @param projections Proyecciones ordenadas
    ...
    int lcut)
{
    ...
    if (np == 0) return candidates;

    // Contadores incrementales
    // Fortran: KSV, KSE, LSV, LSE
    ...
    for (size_t i = 0; i < np; ++i) {
        ...
        if (firstPass) {
            ...
            for (size_t j = i; j < np; ++j) {
                int vote = votes[j];
                if (vote == VoteCode::MISSING) continue;
...
/**
 * @brief Encuentra el mejor corte entre candidatos, manejando empates.
 *
...
/**
 * @brief Evalua todos los cortes para una polaridad.
 *
 * @param projections Proyecciones ordenadas
...
/**
 * @brief Calcula centroides de cada grupo de clasificacion.
 *
 * @param projections Proyecciones ordenadas
 * @param votes Votos reordenados
...
/**
 * @brief Calcula errores de clasificacion por legislador.
 *
 * @param projections Proyecciones ordenadas
 * @param votes Votos reordenados
 * @param originalIndices Mapeo a indices originales
    ...
    CuttingPointMode mode)
{
    ...
    if (mode == CuttingPointMode::NORMAL ||
        ...
        for (int idx : originalIndices) {
            if (idx > maxLegislator) maxLegislator = idx;
        }
